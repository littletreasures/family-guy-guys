...
const STORAGE_KEY = "fg-letterlog:v2";
...
function App() {
  ...
  const [dataset, setDataset] = useState<PodcastDataset>(persisted?.dataset ?? demoDataset);
  const [selectedCohostId, setSelectedCohostId] = useState(
    persisted?.selectedCohostId ?? persisted?.dataset.cohosts[0]?.id ?? demoDataset.cohosts[0].id,
  );
  ...
  useEffect(() => {
    const payload: PersistedState = { dataset, selectedCohostId };
  ...
  return (
    <div className="min-h-screen overflow-hidden text-slate-100">
      <Hero dataset={dataset} metrics={metrics} onNavigate={navigate} onExport={exportDataset} />
      <main className="mx-auto max-w-7xl px-5 pb-16 pt-8 sm:px-6 lg:px-8">
        ...
        {route.page === "episode" ? (
          <EpisodePage
            ...
            onNavigate={navigate}
            onStatus={updateStatus}
            onReview={updateSelectedReview}
          />
        ) : null}

        {route.page === "season" ? (
          <SeasonPage dataset={dataset} season={route.season} onNavigate={navigate} onStatus={updateStatus} />
        ) : null}
...
function CatalogPage({
  ...
  seasons: number[];
  onNavigate: (route: Route) => void;
  onStatus: (episodeId: string, status: WatchStatus) => void;
}) {
  return (
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {episodes.map((episode) => (
            <EpisodeTile
              ...
              onStatus={(status) => onStatus(episode.id, status)}
            />
          ))}
        </div>
...
function EpisodePage({
  ...
  onNavigate: (route: Route) => void;
  onStatus: (episodeId: string, status: WatchStatus) => void;
  onReview: (episodeId: string, updates: Partial<Review>) => void;
}) {
...
function loadPersistedState(): PersistedState | null {
  try {
    ...
    return { dataset: normalizeDataset(parsed.dataset ?? parsed), selectedCohostId: stringValue(parsed.selectedCohostId, "") };
  } catch {
    return null;
  }
}
...
function parseHash(): Route {
  ...
  if (!hash) return { page: "catalog" };
  const [page, value] = hash.split("/");
  if (page === "episode" && value) return { page: "episode", id: decodeURIComponent(value) };
  if (page === "season" && value) return { page: "season", season: Number(value) };
  if (page === "host" && value) return { page: "host", id: decodeURIComponent(value) };
  if (page === "pipeline") return { page: "pipeline" };
  return { page: "catalog" };
}

function routeToHash(route: Route): string {
  if (route.page === "catalog") return "";
  if (route.page === "episode") return `episode/${encodeURIComponent(route.id)}`;
  if (route.page === "season") return `season/${route.season}`;
  if (route.page === "host") return `host/${encodeURIComponent(route.id)}`;
  return "pipeline";
}
